...
) : RecyclerView.Adapter<MySecondAdapter.ViewHolder>() {
    private val dataResults = mutableListOf<GameResult>()

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MySecondAdapter.ViewHolder {
        val item = LayoutInflater.from(parent.context).inflate(R.layout.item_view, parent, false)
        return ViewHolder(item)
    }

    override fun onBindViewHolder(holder: MySecondAdapter.ViewHolder, position: Int) {
